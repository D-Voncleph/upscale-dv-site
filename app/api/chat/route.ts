...
export async function POST(req: Request) {
    ...
    console.log("API Key Present:", !!process.env.GOOGLE_GENERATED_API_KEY);

    if (!process.env.GOOGLE_GENERATED_API_KEY) {
        console.error("ERROR: GOOGLE_GENERATED_API_KEY is missing");
        return new Response("Missing API Key", { status: 500 });
    }

    try {
        const result = streamText({
            model: google('gemini-flash-latest'), // Using available flash alias
